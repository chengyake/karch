...
def fill_data(idx, offset):
    ...
    y,_a, _b, _c = get_percent(idx, offset)
    if y > 0.1:
        y=0.1
    if y < -0.1:
        y=-0.1
...
def main():
    ...
    x = np.fromfile("x_test.bin", dtype=np.uint8) 
    y = np.fromfile("y_test.bin", dtype=np.uint8) 
    
    x = x.reshape((batch2, width, height, channel))
    y = y.reshape((batch2, 1))
